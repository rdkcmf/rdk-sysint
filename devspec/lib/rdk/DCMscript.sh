...
if [ "$LOG_PATH" == "/opt/logs" ]; then 
TELEMETRY_TEMP_RESEND_FILE="/opt/logs/.temp_resend.txt" 
TELEMETRY_INOTIFY_FOLDER="/opt/logs/" 
else
...
if [ "$LOG_PATH" == "/opt/logs" ]; then  
      estbMacAddress=`ifconfig eth0 | grep HWaddr | cut -c39-55`
    echo JSONSTR: $estbMacAddress
    CURL_CMD="curl -w "%{http_code}" '$URL?estbMacAddress=$estbMacAddress&model=$MODEL_NAME'  -o $DCMRESPONSE >> /tmp/telehttpcode.txt "
else
...
do
    if [ "$LOG_PATH" == "/opt/logs" ]; then  
    estbIp=`ifconfig eth0 | grep -i inet | cut -d ":" -f2 | cut -d " " -f1`
else
