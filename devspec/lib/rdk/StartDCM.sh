...
then
 ...
 if [ -f "$TELEMETRY_JSON_RESPONSE" ]; then
    ...
    sh /lib/rdk/DCMscript.sh $DCM_LOG_SERVER $DCM_LOG_SERVER_URL $LOG_SERVER 0 1  >> /opt/logs/telemetry.log &
else
    ...
    do                                                                                                            
       ...
       if [ -f "/opt/rtl_json.txt" ] ||  [ -f "/nvram/rtl_json.txt" ]; then                                                                      
         echo "files exists !!!!..."                                                                              
       #  sh $RDK_PATH/DCMscript-log.sh $DCM_LOG_SERVER $DCM_LOG_SERVER_URL $LOG_SERVER 0 1  >> /opt/logs/logupload.log &
         sleep 5
