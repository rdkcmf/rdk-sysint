...
useDirectRequest()
{
       ...
       if [ "$uploadProtocoltel" != "TFTP" ]; then
         ...
         if [ "$LOG_PATH" == "/opt/logs" ]; then  
           CURL_CMD="curl --tlsv1.2 -w '%{http_code}\n' -H \"Accept: application/json\" -H \"Content-type: application/json\" -X POST -d @/opt/rtl_json.txt '$HTTPURL' --connect-timeout 30 -m 30"
          else
	   ...
	   if [ $ret -eq 0 ]; then
           ...
           if [ -f "rtl_json.txt" ]; then
              ...
              if [ "$LOG_PATH" == "/opt/logs" ]; then 
              RpiMacAddress=`ifconfig eth0 | grep HWaddr | cut -c39-55`
              else
