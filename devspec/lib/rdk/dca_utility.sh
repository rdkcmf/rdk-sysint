...
if [ ! -f $SORTED_PATTERN_CONF_FILE ]; then
   ...
   if [ "$LOG_PATH" == "/opt/logs" ]; then      
         if $singleEntry ; then
            outputJson="$outputJson{\"Profile\":\"RDKV\"},{\"mac\":\"$estbMac\"},{\"erouterIpv4\":\"$erouterIpv4\"},{\"erouterIpv6\":\"$erouterIpv6\"},{\"PartnerId\":\"$partnerId\"},{\"AccountId\":\"$accountId\"},{\"Version\":\"$firmwareVersion\"},{\"Time\":\"$cur_time\"}"
            singleEntry=false
         else
            outputJson="$outputJson,{\"Profile\":\"RDKV\"},{\"mac\":\"$estbMac\"},{\"erouterIpv4\":\"$erouterIpv4\"},{\"erouterIpv6\":\"$erouterIpv6\"},{\"PartnerId\":\"$partnerId\"},{\"AccountId\":\"$accountId\"},{\"Version\":\"$firmwareVersion\"},{\"Time\":\"$cur_time\"}"
         fi
